 ...
 * @link      $HeadURL: https://open.logics.net.au/buildtools/codesniffer/tags/0.1.3/src/CodeSniffer/Tokenizers/CSSTokenizer.php $
 */

class CSSTokenizer extends PHPTokenizer
    {
	 ...
	 * @internalconst T_EMBEDDED_PHP        T_EMBEDDED_PHP token
	 ...
	 * @internalconst T_URL                 T_URL token
	 * @internalconst T_ASPERAND            T_ASPERAND token
	 *
	 * @untranslatable ^PHPCS_CSS_T_CLOSE_TAG^
	 * @untranslatable ^PHPCS_CSS_T_OPEN_TAG^
	 * @untranslatable T_OPEN_TAG
	 * @untranslatable PHPCS_CSS_T_OPEN_TAG
	 * @untranslatable PHPCS_CSS_T_CLOSE_TAG
	 * @untranslatable T_EMBEDDED_PHP
	 * @untranslatable <?php
	 * @untranslatable T_COLON
	 * @untranslatable T_COLOUR
	 * @untranslatable T_HASH
	 * @untranslatable T_STRING
	 * @untranslatable T_STYLE
	 * @untranslatable url
	 * @untranslatable T_URL
	 */

	public function tokenizeString($string, $eolChar = '\n')
	    {
		Console::report("*** " . _("START CSS TOKENIZING") . " ***", 1, 1);
		...
		for ($stackPtr = 1; $stackPtr < $numTokens; $stackPtr++)
			...
			Console::report(_("Process token") . " " . $stackPtr . ": " . $token["type"] . " => " . Console::prepareForOutput($token["content"]), 1, 1);

			if ($token["code"] === T_POWER && $tokens[($stackPtr + 1)]["content"] === "PHPCS_CSS_T_OPEN_TAG")
			    {
				$content = "<?php";
				for ($i = ($stackPtr + 3); $i < $numTokens; $i++)
				    {
					if ($tokens[$i]["code"] === T_POWER && $tokens[($i + 1)]["content"] === "PHPCS_CSS_T_CLOSE_TAG")
					    {
				...
				Console::report("=> " . _("Found embedded PHP code") . ": " . Console::prepareForOutput($content), 2, 1);
			...
			switch ($token["code"])
				...
				case T_COLON:
					...
					for ($x = ($stackPtr - 1); $x >= 0; $x--)
					    {
						if (isset(Tokens::$emptyTokens[$finalTokens[$x]["code"]]) === false)
						    {
							break;
						    }
					    } //end for
				...
				case T_STRING:
					if (strtolower($token["content"]) === "url")
						...
						for ($y = ($x + 1); $y < $numTokens; $y++)
						    {
							if (isset(Tokens::$emptyTokens[$finalTokens[$y]["code"]]) === false)
							    {
								break;
							    }
						    } //end for
		...
		Console::report("*** " . _("END CSS TOKENIZING") . " ***", 1, 1);
	...
	public function processAdditional(array &$tokens, $eolChar)
	    {
		// We override this method because we don't want the PHP version to run during CSS processing because it is wasted processing time.
